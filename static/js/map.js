...
async function loadReceitasLayerForLayer(layer) {
        ...
        async function loadMunicipalityBoundariesForRevendasLayer(layer, layerData, minMax) {
            try {
                ...
                console.log('Territory municipalities:', Array.from(territoryMunicipalities));
                ...
                const filteredFeatures = geoData.features.filter(feature => {
                    ...
                    if (isInTerritory) {
                        console.log(`Found municipality in territory: ${municipalityCode} - ${feature.properties.NOME || feature.properties.NM_MUN || feature.properties.nm_mun}`);
                    }
                    return isInTerritory;
                });

                console.log(`Filtered ${filteredFeatures.length} municipalities for revenda territory`);

                if (filteredFeatures.length === 0) {
                    console.error('Nenhum município encontrado no território da revenda');
                ...
                if (layer.visible !== false) {
                    mapLayer.addTo(map);
                    console.log('Revenda layer added to map with', filteredFeatures.length, 'municipalities');
                }
        ...
        function getFeatureStyleForRevendasLayer(feature, layer) {
            // Para revendas, sempre mostrar as bordas dos municípios do território
            // com uma cor visível e preenchimento claro
            return {
                fillColor: layer.color || '#FF5722',  // Cor da camada ou padrão
                weight: 2,                            // Borda visível
                opacity: 1,                           // Borda totalmente opaca
                color: layer.color || '#FF5722',      // Cor da borda
                fillOpacity: 0.3,                     // Preenchimento visível mas claro
                dashArray: null                       // Linha contínua
            };
        }
